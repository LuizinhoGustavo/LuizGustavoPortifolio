    ...
    <div className="flex flex-row bg-dotted-pattern justify-center">
      <div className="absolute flex items-center gap-3 top-4 right-4 z-50">
        ...
        {window.localStorage.getItem('i18nextLng') === 'pt-BR' ? (
          <button
            ...
            className="bg-transparent text-textColor text-sm flex items-center gap-1"
          >
            {' '}
            <img src={usa} alt="Bandeira Estados Unidos" />
            English
          </button>
        ) : (
          <button
          ...
          >
            {' '}
            <img src={brazil} alt="Bandeira Brasil" />
            Português
          </button>
        )}
      </div>
      ...
      <div id="home" className="flex flex-col w-full -translate-x-1/5">
        <div className="h-screen flex mx-auto flex-col gap-2 items-center justify-center max-w-[1200px]">
          <h1 className="font-bold text-textColor text-2xl md:text-5xl">
            <ReactTyped
              strings={['Luiz Gustavo', i18n.t('title.dev')]}
              typeSpeed={200}
              backSpeed={30}
              loop
            />
          ...
          <div className="flex w-full flex-row justify-center gap-14">
            <a href="https://www.linkedin.com/in/silva-luizg/" target="_blank">
              <Linkedin
                size={40}
                strokeWidth={1.7}
                className="text-iconsColor hover:text-orange-600  duration-500"
              />
            </a>
        ...
        <div className="flex flex-col gap-10 justify-start mx-auto w-full max-w-[850px]">
          ...
          <div className="mx-3 grid gap-10 grid-cols-1 lg:grid-cols-2 mb-32">
            <Project
              title={`${i18n.t('projects.randomGutenbergBook.title')}`}
              description={`${i18n.t('projects.randomGutenbergBook.description')}`}
              projectLink="https://randomgutenbergbook.vercel.app/"
              filePic={FilePic}
            />
          </div>
          ...
          <div className="flex flex-col gap-10 w-full py-14">
            ...
            <div className="flex justify-center mx-auto gap-12 flex-wrap w-[90%] pb-16">
              <div className="bg-[#DB6B6B50] h-72 w-52 flex flex-col justify-center border-2 border-[#DB6B6B] rounded-2xl -rotate-[10deg] p-3">
                ...
                <span className="font-bold text-textColor text-[1.4rem]">
                  {`${i18n.t('whatIDo.frontend')}`}{' '}
                </span>
              </div>
          ...
          <div className="flex flex-col gap-2 w-2/3 pb-32 mx-auto">
            <TimelineElement
              ...
              title={`${i18n.t('experience.english')}`}
              date="2014 - 2024"
              place={`${i18n.t('experience.place.culture')}`}
            />
            <TimelineElement
              representation="GraduationCap"
              title={`${i18n.t('experience.technicalCourse')}`}
              date="2021 - 2023"
              place={`${i18n.t('experience.place.etec')}`}
            />
            <TimelineElement
              representation="GraduationCap"
              title={`${i18n.t('experience.bachelor')}`}
              date="2024 - 2028"
              place={`${i18n.t('experience.place.ufabc')}`}
            />
            <TimelineElement
              representation="LaptopMinimal"
              title={`${i18n.t('experience.internship')}`}
              date="2024 - Atualmente"
              place={`${i18n.t('experience.place.cityHall')}`}
            />
            <TimelineElement
              representation="Plane"
              title={`${i18n.t('experience.exchange')}`}
              date="Jan 2026 - Fev 2026"
              place={`${i18n.t('experience.place.exchange')}`}
            />
          </div>
        </div>
      </div>
